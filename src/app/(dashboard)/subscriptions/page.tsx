...
function DonutChart({ subscriptions }: { subscriptions: Subscription[] }) {
  const slices = useMemo(() => {
    ...
    return Object.entries(map)
      ...
      .map(([name, val]) => {
        ...
        return { name, val, pct: Math.round((val / total) * 100), angle, start, color: PALETTE[ci] };
      });
  }, [subscriptions]);

  const cx = 90, cy = 90, r = 68, ri = 44;
  ...
  const monthlyTotal = subscriptions.reduce((s, x) =>
    s + (x.type === "MONTHLY" ? x.amount : Math.round(x.amount / 12)), 0);
    ...
    <div className={styles.chartCard}>
      ...
      <div className={styles.donutWrap}>
        <svg viewBox="0 0 180 180" width={160} height={160} style={{ flexShrink: 0 }}>
          ...
          <text x={cx} y={cy - 8} textAnchor="middle" fontSize="10" fill="var(--text-muted)">월 합계</text>
          <text x={cx} y={cy + 8} textAnchor="middle" fontSize="12" fontWeight="600" fill="var(--text-primary)">
...
function BarChart({ subscriptions }: { subscriptions: Subscription[] }) {
  const bars = useMemo(() => {
    ...
    return sorted.map(s => ({ ...s, pct: (s.monthly / max) * 100, ci: colorIdx(s.providerName) }));
  }, [subscriptions]);
      ...
      <div className={styles.barList}>
        {bars.map((b, i) => (
          <div key={i} className={styles.barRow}>
            <span className={styles.barLabel} title={b.name}>{b.name}</span>
...
const EMPTY_FORM: SubscriptionPayload = {
  name: "", amount: 0, type: "MONTHLY", providerId: "", description: "",
};
...
export default function SubscriptionsPage() {
  ...
  function openEdit(sub: Subscription) {
    setForm({ name: sub.name, amount: sub.amount, type: sub.type, providerId: sub.providerId, description: sub.description ?? "" });
    setFormError("");
  ...
  return (
    <div className={styles.wrap}>
      ...
      <div className={styles.header}>
        ...
        <button className={styles.btnAdd} onClick={openCreate}>+ 구독 추가</button>
      </div>
      ...
      {summary && (
        <div className={styles.summaryGrid}>
          ...
          <div className={styles.summaryCard}>
            ...
            <span className={styles.summarySub}>월결제 {summary.monthlyCount} · 년결제 {summary.yearlyCount}</span>
          </div>
        </div>
      )}
              ...
              {sorted.map((sub) => {
                ...
                return (
                  <tr key={sub.id}>
                    ...
                    <td>
                      <span className={`${styles.badge} ${sub.type === "MONTHLY" ? styles.monthly : styles.yearly}`}>
                        {TYPE_LABEL[sub.type]}
                      </span>
                    </td>
                    ...
                    <td className={styles.actionsCell}>
                      <button className={styles.btnEdit} onClick={() => openEdit(sub)}>수정</button>
                      <button className={styles.btnDel} onClick={() => handleDelete(sub)}>삭제</button>
                    </td>
                  </tr>
                );
              })}
        ...
        <Modal title={modal.mode === "create" ? "구독 추가" : "구독 수정"} onClose={() => setModal(null)}>
          <form onSubmit={handleSubmit} className={f.form}>
            ...
            <div className={f.field}>
              ...
              <input className={f.input} value={form.name} onChange={e => setField("name", e.target.value)}
                placeholder="예: Netflix 프리미엄" required autoFocus maxLength={200} />
            </div>
            <div className={f.row}>
              <div className={f.field}>
                <label className={f.label}>금액 (원)</label>
                <input type="number" className={f.input} value={form.amount || ""} min={1}
                  onChange={e => setField("amount", Number(e.target.value))} placeholder="13500" required />
              </div>
              <div className={f.field}>
                <label className={f.label}>결제 타입</label>
                <select className={f.select} value={form.type}
            ...
            <div className={f.field}>
              ...
              <select className={f.select} value={form.providerId}
                onChange={e => setField("providerId", e.target.value)} required>
            ...
            <div className={f.field}>
              ...
              <textarea className={f.textarea} value={form.description} maxLength={500}
                onChange={e => setField("description", e.target.value)} placeholder="추가 설명을 입력하세요" />
            </div>
